...
def ax_plot_polymer_config_rectangle(ax, filename, elev=30, zaim=30):
    ...
    for i in range(len(x)-1):
        # entire rectangle
        ax.plot([x[i]-0.5*d*vx[i], x[i]+0.5*d*vx[i], x[i]+0.5*d*vx[i]+ux[i], x[i]-0.5*d*vx[i]+ux[i], x[i]-0.5*d*vx[i]], [y[i]-0.5*d*vy[i], y[i]+0.5*d*vy[i], y[i]+0.5*d*vy[i] + uy[i], y[i]-0.5*d*vy[i]+uy[i], y[i]-0.5*d*vy[i]], [z[i]-0.5*d*vz[i], z[i]+0.5*d*vz[i], z[i]+0.5*d*vz[i]+uz[i], z[i]-0.5*d*vz[i]+uz[i], z[i]-0.5*d*vz[i]], color="black", linewidth=1, alpha=0.7)
        # colored edge indicating flipping
        ax.plot([x[i]-0.5*d*vx[i], x[i]+0.5*d*vx[i]], [y[i]-0.5*d*vy[i], y[i]+0.5*d*vy[i]], [z[i]-0.5*d*vz[i], z[i]+0.5*d*vz[i]], color="black", linewidth=1, alpha=0.7)
...
def ax_plot_2d_canel_nonflip_2d(ax, labelon, shifts, rots):
    ...
    ax.plot([e3[0], e4[0]], [e3[1], e4[1]], color="black", linewidth=1, alpha=1, solid_capstyle='round', zorder=100)
    if (labelon):
        ax.plot([e1[0], e4[0]], [e1[1], e4[1]], color="blue", linewidth=2, alpha=1, solid_capstyle='round', zorder=200, label="Sync")
    else:
...
def ax_plot_2d_canel_flip_2d(ax, labelon, shifts, rots):
    print("sample plotting 2d canal, flip")
    atom_file = "./2D_CANEL_corrd/2d_canal2f_atom.txt"
    bond_file = "./2D_CANEL_corrd/2d_canal2f_bond.txt"
    atom_data = pd.read_csv(atom_file, sep=" ", skiprows=1)
...
def ax_plot_sample_MC_config_2d(ax, filename, shifts, rots):
    ...
    for i in range(len(x)-1):
        ...
        ax.plot(xs, ys, color="black", linewidth=1, alpha=0.4 + (z[i]-zmin)/(zmax-zmin)*0.4, solid_capstyle='round')
    ...
    for i in range(1, len(flip)-1):
        ...
        ax.plot([x[i]-0.5*d*vx[i], x[i]+0.5*d*vx[i]], [y[i]-0.5*d*vy[i], y[i]+0.5*d*vy[i]], color=color, lw=2, alpha=0.4 + (z[i]-zmin)/(zmax-zmin)*0.4, solid_capstyle='round')  # v
...
def plot_molecule_structure_model_demo(tex_lw=240.71031, ppi=72):
    ...
    ax_plot_2d_canel_flip_2d(ax, False, [27, -11, 0], [rotx+0.7, roty, 0.7])
